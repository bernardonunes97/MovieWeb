   ...
   <body>
	<header>
		<nav class="navbar navbar-inverse bg-dark navbar-dark">
			...
			<div class="col-9">
				<input id="txtSearch" class="form-control mr-sm-2" type="search"
					placeholder="Search" aria-label="Search">
			</div>
			<div class="col">
				<a class="navbar-brand" href="Favoritos.jsp"><i
					class="fas fa-star"></i></a>
				<c:choose>
					<c:when test="${username == null }">
						<a class="navbar-brand" href="Login.jsp"><i
							class="fas fa-user-alt"></i> Entrar</a>
					</c:when>
					<c:otherwise>
						<a class="navbar-brand" href="Login.jsp"><i
							class="fas fa-user-alt"></i> ${username}</a>
					</c:otherwise>
				</c:choose>
			</div>
		</nav>
		...
		<c:forEach var="movie" items="${controller.loadMovies(actualPage)}">
			<form action="movie" method="post">
				<div class="card bg-dark text-white">
					...
					<div class="card-body">
						<p class="card-title">
							<i class="fas fa-star"></i><b> ${movie.vote}/10</b>
						</p>
						...
						<c:choose>
							<c:when test="${movie.isFavorite}">
								<input type="submit" class="btn btn-danger favorite"
									value="Dislike">
							</c:when>
							<c:otherwise>
								<input type="submit" class="btn btn-outline-primary favorite"
									value="Like">
							</c:otherwise>
						</c:choose>
					</div>
	...
	<footer>
		<form action="movie">
			<ul class="pagination">
				<li class="page-item">
					<button class="page-link" aria-label="Previous" name="actualPage"
						value="${actualPage - 1}">
					...
					<c:choose>
						<c:when test="${index == actualPage}">
							<li class="page-item active"><input class="page-link"
								type="submit" name="actualPage" value="${index}"></li>
						</c:when>
						<c:otherwise>
							<li class="page-item"><input class="page-link" type="submit"
								name="actualPage" value="${index}"></li>
						</c:otherwise>
					</c:choose>
				</c:forEach>
				<li class="page-item">
					<button class="page-link" aria-label="Next" name="actualPage"
						value="${actualPage + 1}">
